...
export function VelioDashboard() {
  ...
  const [activeView, setActiveView] = useState<"channels" | "videos">(
    "channels",
  );
  ...
  useEffect(() => {
    if (activeFolder) {
      async function loadVideos() {
        setLoading(true);
        const data = await VelioClient.getFolderVideos(
          activeFolder as number,
        ).catch(() => []);
  ...
  useEffect(() => {
    if (activeFolder) {
      DiscoveryClient.getFolderChannels(activeFolder)
        .then(setFolderChannels)
        .catch(() => setFolderChannels([]));
    }
  ...
  const handleCreateFolder = async () => {
    ...
    const tags = newFolderTags
      ...
      .map((t) => t.trim())
      .filter(Boolean);
    const newFolder = await VelioClient.createFolder(folderName, tags).catch(
      () => null,
    );
    if (newFolder) {
  ...
  const sortedChannels = [...folderChannels].sort((a, b) => {
    if (channelSort === "avg_views")
      return (b.avg_views_per_video ?? 0) - (a.avg_views_per_video ?? 0);
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    className="bg-synthwave-magenta hover:bg-fuchsia-600 h-9 w-9 synth-glow-magenta text-white"
                  >
                ...
                {folders.length === 0 ? (
                  ...
                  folders.map((folder) => (
                    ...
                    >
                      <Button
                        variant={
                          activeFolder === folder.id ? "secondary" : "ghost"
                        }
                        ...
                        {folder.tags && folder.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-0.5">
                            {folder.tags.map((tag: string) => (
                              <span
                                key={tag}
                                className="text-[10px] px-1.5 py-0 rounded bg-slate-700 text-slate-400"
                              >
                                {tag}
                              </span>
                            ))}
          ...
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-400">
                Latest videos from all channels in this folder
              </p>
              <div className="flex gap-2 mt-2">
                <Button
                  ...
                  className={
                    activeView === "channels"
                      ? "bg-synthwave-magenta/20 text-synthwave-magenta border border-synthwave-magenta/50"
                      : "text-slate-400"
                  }
                >
            ...
            {activeFolder && (
              <p className="text-xs text-slate-500">
                Use{" "}
                <strong className="text-synthwave-cyan">
                  Discover Channels
                </strong>{" "}
                to add channels here
              </p>
            )}
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="py-20 text-center text-slate-500">
                Select or create a folder to view the feed.
              </div>
            ) : (
              <>
                {activeView === "channels" &&
                  ...
                  (folderChannels.length === 0 ? (
                    <div className="py-16 text-center text-slate-500 text-sm">
                      No channels in this folder yet. Use{" "}
                    ...
                    <div className="space-y-3">
                      ...
                      {sortedChannels.map((ch) => (
                        <div
                          key={ch.id}
                          className="flex items-center gap-4 p-3 rounded-lg bg-slate-900/50 border border-slate-800 hover:border-synthwave-magenta transition-colors group"
                        >
                          {ch.thumbnail_url ? (
                            <img
                          ...
                          <button
                            ...
                            onClick={async () => {
                              await DiscoveryClient.removeChannelFromFolder(
                                activeFolder as number,
                                ch.id,
                              );
                              setFolderChannels((prev) =>
                ...
                {activeView === "videos" &&
                  ...
                  (folderVideos.length === 0 ? (
                    <div className="py-20 text-center text-slate-500">
                      No videos found. Map channels to this folder to start
                      tracking.
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {folderVideos.map((video, idx) => (
                        <div
                          key={idx}
                          className="flex gap-4 p-3 rounded-lg bg-slate-900/50 border border-slate-800 hover:border-synthwave-magenta transition-colors group cursor-pointer"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-slate-200 line-clamp-2 leading-snug group-hover:text-synthwave-magenta transition-colors">
                              {video.title}
                            </p>
